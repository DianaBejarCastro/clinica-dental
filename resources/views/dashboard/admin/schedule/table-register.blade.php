...
<table id="dentists-table" class="min-w-full bg-white max-h-96" width="100%">
    <thead class="bg-gray-100 text-gray-600 uppercase text-sm leading-normal">
        <tr>
            ...
            <th class="py-2 px-2 text-left">Acciones</th> <!-- Espacio para los botones "Editar" y "Eliminar" -->
        </tr>
    </thead>
    <tbody class="text-gray-600 text-sm font-light">
        @foreach ($dentists as $dentist)
            <tr class="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td class="py-2 px-2">
                    <button onclick="editSchedule('{{ $dentist->id }}')"
...
<script>
    ...
    function editSchedule(dentistId) {
fetch('{{ route('schedule.setEditId') }}', {
    method: 'POST',
    headers: {
...
.then(data => {
    if (data.status === 'success') {
        window.location.href = '{{ route('schedule.edit.view') }}';
    } else {
