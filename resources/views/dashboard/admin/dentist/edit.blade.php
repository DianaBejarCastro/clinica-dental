...
<div class="container mx-auto p-4 sm:p-2">
    <div class="flex flex-col items-center mb-4 md:flex-row md:justify-between md:items-center">
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 leading-tight mb-4 shadow-sm">
            Editar Datos del Dentista
        </h1>
    </div>

    <h2 class="text-sm sm:text-base text-gray-600 mb-4">Editar Contraseña</h2>
    <form class="w-full max-w-6xl bg-white p-8 rounded-lg shadow-md" method="POST" action="{{ route('dentist.changePassword') }}">
        @csrf
        <div class="mt-4">
    ...
    <div class="flex justify-center">
        ...
        method="POST" action="{{ route('dentist.update') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="ci" name="ci" value="{{ old('ci', $dentist->ci) }}" placeholder="CI" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    @error('ci')
                    <p class="text-red-500 mt-2">{{ $message }}</p>
    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="day_of_birth" name="day_of_birth" value="{{ old('day_of_birth', $dentist->day_of_birth) }}" placeholder="Fecha de nacimiento" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    @error('day_of_birth')
                    <p class="text-red-500 mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="address" name="address" value="{{ old('address', $dentist->address) }}" placeholder="Dirección" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    @error('address')
                    <p class="text-red-500 mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $dentist->phone) }}" placeholder="Numero de Telefono" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    @error('phone')
                    <p class="text-red-500 mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="center_id" name="center_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                        ...
                        @foreach($centers as $center)
                            <option value="{{ $center->id }}" {{ $center->id == old('center_id', $dentist->center_id) ? 'selected' : '' }}>{{ $center->name_branch }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <!-- Specialty Field -->
            <div class="mb-4 mt-4">
                <x-label for="specialties" class="mb-2" value="{{ __('Especialidades') }}" />
                <div id="specialties" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($specialties as $specialty)
                        <label class="flex items-center">
                            <input type="checkbox" name="specialties[]" value="{{ $specialty->id }}"
                            @if($dentist->specialties->contains($specialty->id)) checked @endif
