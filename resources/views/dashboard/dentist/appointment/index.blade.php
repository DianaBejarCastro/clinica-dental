...
@section('content-dashboard')
            ...
            <table class="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2">Nombre y Estado</th>
    ...
    <script>
        ...
        var updateRoute = "{{ route('dentist.appointments.update', ['id' => ':id']) }}";
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetchAppointmentsDentist(urlDate);

            function fetchAppointmentsDentist(date) {
                fetch(`/dentist/appointments/${date}`)
                    .then(response => response.json())
                    .then(data => {
                        ...
                        data.appointments.forEach(appointment => {
                            ...
                            timeCell.textContent = appointment.start_time;
                            row.appendChild(timeCell);

                            const nameStatusCell = document.createElement('td');
                            nameStatusCell.className = 'border px-4 py-2';
                            const statusColor = appointment.status === 'atendido' ? 'text-yellow-600' :
                                appointment.status === 'reservado' ? 'text-green-600' : 'text-red-600';
                            nameStatusCell.innerHTML =
                                `${appointment.user.name}<br>(Dr.(a). ${appointment.dentist.user.name})<br><span class="${statusColor}">${appointment.status.charAt(0).toUpperCase() + appointment.status.slice(1)}</span>`;
                            row.appendChild(nameStatusCell);

                            const actionCell = document.createElement('td');
                            actionCell.className = 'border px-4 py-2';

                            if (appointment.status === 'reservado' || appointment.status ===
                                'modificado') {
                                ...
                                editButton.setAttribute('data-appointment-id', appointment.id);
                                editButton.setAttribute('data-start-time', appointment.start_time);
                                editButton.className = 'edit-appointment-btn';
                                editButton.onclick = function() {
                                    editAppointment(appointment.id, appointment.date, appointment
                                        .start_time);
                                };
                                ...
                                const cancelButton = document.createElement('button');
                                cancelButton.className = 'open-modal-button';
                                cancelButton.onclick = function() {
                                    openAppointmentModal(appointment.user_id);
                                };
                                cancelButton.innerHTML =
            ...
            function actualizar() {
                ...
                fetchAppointmentsDentist(selectedDate);
            }
        });
    </script>
